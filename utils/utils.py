...
from config import RESULT_PATH, MODEL_PATH
...
def get_result_path(dataset_name, network_arch, random_seed, result_subfolder, source_class, sink_class):
    if not os.path.isdir(RESULT_PATH):
        os.makedirs(RESULT_PATH)
    ISOTIMEFORMAT='%Y-%m-%d_%X'
    t_string = '{}'.format(time.strftime( ISOTIMEFORMAT, time.gmtime(time.time()) ))
    result_path = os.path.join(RESULT_PATH, result_subfolder, "{}_{}_{}_{}_source{}_sink{}".format(t_string, dataset_name, network_arch, random_seed, len(source_class), len(sink_class)))
    os.makedirs(result_path)
    return result_path
